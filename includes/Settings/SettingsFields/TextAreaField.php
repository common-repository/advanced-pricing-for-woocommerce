...
class TextAreaField extends FieldBase {
	...
	public function get_field_html( $return = false ) {
		...
		?>
		<textarea <?php $this->field_id(); ?> <?php $this->field_classes( 'large-text' ); ?> <?php $this->field_name(); ?> <?php $this->custom_attributes_html(); ?> ><?php echo ( ( ! empty( $this->field['html_allowed'] ) && $this->field['html_allowed'] ) ? wp_kses_post( $this->field['value'] ) : esc_html( $this->field['value'] ) ); ?></textarea>
		<?php
		if ( $return ) {
			return ob_get_clean();
		}
