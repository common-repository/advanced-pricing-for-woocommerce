...
abstract class FieldBase extends Base {
	...
	protected function custom_attributes_html( $attrs_key = 'attrs', $additional_attrs = array() ) {
		$attributes = '';
		$attrs = ( ! empty( $this->field[ $attrs_key ] ) && is_array( $this->field[ $attrs_key ] ) ) ? $this->field[ $attrs_key ] : array();
		$attrs = array_replace( $attrs, $additional_attrs );
	...
	public function add_field( $echo = true ) {
		...
		<?php if ( ! empty( $this->field['input_label'] ) ) : ?>
			<div class="<?php echo esc_attr( $is_inline ? ( $this->field['label_wrapper_width'] ?? 'col-lg-3' ) : 'col-12' ); ?> d-flex flex-column <?php echo esc_attr( ! empty( $this->field['center_label'] ) ? 'align-items-center' : 'align-items-start' ); ?> mb-4 mb-md-0 <?php echo esc_attr( $this->field['label_wrapper_classes'] ?? '' ); ?>">
				...
				<?php endif; ?>
				<?php do_action( $this->id . '-after-settings-field-label-' . $this->field['filter'], $this->field ); ?>
			</div>
		<?php endif; ?>

		<!-- Field input -->
		<div class="<?php echo esc_attr( ! empty( $this->field['input_label'] ) ? ( $is_inline ? ( $this->field['input_wrapper_width'] ?? 'col-lg-9' ) : 'col-12' ) : 'col-12' ); ?> d-flex flex-column align-items-start <?php echo esc_attr( $this->field['input_wrapper_classes'] ?? '' ); ?>">
			<div class="input w-100">
				...
				<!-- Input  -->
				<div class="<?php echo esc_attr( 'repeater' === $this->field['type'] ? 'repeater-' : '' ); ?>input-field-container <?php echo esc_attr( 'repeater' === $this->field['type'] ? $this->id . '-' . $this->field['key'] . '-repeater-container' : '' ); ?> <?php echo esc_attr( $this->field['input_field_wrapper_classes'] ?? '' ); ?>">
					<?php
