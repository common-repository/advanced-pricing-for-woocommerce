...
abstract class AdminPage extends Base {
	...
	public function is_current_page( $custom_slug = null ) {
		$full_url = ( is_ssl() ? 'https://' : 'http://' ) . ( ! empty( $_SERVER['HTTP_HOST'] ) ? sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) ) : '' ) . ( ! empty( $_SERVER['REQUEST_URI'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '' );
	...
	public function assets( $suffix ) {
		if ( $this->is_current_page() || ( get_plugin_page_hookname( $this->page_props['menu_slug'], $this->page_props['parent_slug'] ) === $suffix ) ) {
			$this->set_assets();
			...
			do_action( self::$plugin_info['name'] . '-' . $this->page_props['menu_slug'] . '-admin-page-assets', $this );
		}
	}
	...
	public function is_tab_active( $tab_name, $tab_arr = array() ) {
		...
		if ( ! empty( $_GET[ $this->page_props['tab_key'] ] ) && sanitize_text_field( wp_unslash( $_GET[ $this->page_props['tab_key'] ] ) ) === $tab_name ) {
			return true;
		}
